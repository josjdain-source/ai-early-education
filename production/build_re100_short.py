...
PLAN=json.load(open(f"{HERE}/re100_short_plan.json",encoding="utf-8"))
KEY=PLAN["id"]; SPEED=PLAN.get("speed",1.35); TAIL=2.0; BRAND=PLAN.get("brand","광주 반도체 시나리오")
STYLE=PLAN["style_token"]; NEG=PLAN["negative"]
A=f"{REPO}/assets/re100-belt"; RENDER=f"{A}/render"; IMG=f"{A}/simg"; FR=f"{A}/sframe"; AUD=f"{A}/saud"
...
def sdxl(scene,seed,out):
    g={"4":{"class_type":"CheckpointLoaderSimple","inputs":{"ckpt_name":CKPT}},
       ...
       "9":{"class_type":"SaveImage","inputs":{"filename_prefix":"reSH","images":["8",0]}}}
...
def register_to_studio(mp4, poster):
    ...
    vid="re100-belt-short"
    rel=os.path.relpath(mp4,REPO).replace("\\","/"); prel=os.path.relpath(poster,REPO).replace("\\","/") if poster else ""
    desc=("반도체도 AI도 결국 전기 싸움. 광주(칩)+솔라시도(AI 연산)+전남 재생에너지(전기) — 호남 RE100 산업벨트가 한 세트인 이유. ★솔라시도는 구상 단계(3GW·15조 보도 기준)이며 공식 확정이 아닙니다."
          +chr(10)+chr(10)+"수치 출처: 전남도·언론 보도(세계 최대 3GW·2030년까지 15조 구상, 북버지니아 2.5GW 비교)"
          +chr(10)+chr(10)+"#광주 #반도체 #RE100 #솔라시도 #AI데이터센터 #전남 #Shorts")
    entry={"video_id":vid,"title":PLAN.get("final_title","광주 반도체, 공장만 지으면 끝이 아닙니다"),"video_type":"short",
           "series":"S08_region_scenario","mp4_path":rel,"poster_path":prel,"thumbnail_path":prel,
           "tags":["광주","반도체","RE100","솔라시도","AI데이터센터","전남"],"shorts":True,"category":"25",
           ...
           "source":"production/re100_short_plan.json","note":"RE100 3점 세트. 솔라시도=구상 단계 정직 표기. 업로드는 사람 승인."}
...
if __name__=="__main__":
    ...
    for bi,bt in enumerate(beats):
        ...
        if bt.get("image_file"):
            ...
            if not os.path.exists(ip): sdxl(bt["prompt"],8100+bi,ip)
