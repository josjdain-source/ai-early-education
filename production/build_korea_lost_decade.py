#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""중국 AI교육 쇼츠 5편 빌드 — 플랜(korea_lost_decade_shorts_plan.json) 기반.
독립 후킹구조(hook/problem/fact/parent/cta). 1080x1920. 상단 큰 후킹문구·중앙 일러스트·하단 자막·빠른 디졸브.
산출: assets/korea-lost-decade/render/shorts/korea-lost-decade-<s>.mp4"""
...
A=f"{REPO}/assets/korea-lost-decade"; RENDER=f"{A}/render/shorts"; IMG=f"{A}/simg"; FR=f"{A}/sframe"; AUD=f"{A}/saud"
for d in (RENDER,IMG,FR,AUD): os.makedirs(d,exist_ok=True)
KB=r"C:\Windows\Fonts\malgunbd.ttf"; KF=r"C:\Windows\Fonts\malgun.ttf"
PLAN=json.load(open(f"{HERE}/korea_lost_decade_shorts_plan.json",encoding="utf-8")); STYLE=PLAN["style"]
NEG=("photo, photorealistic, 3d render, text, letters, words, korean text, chinese text, watermark, logo, "
 ...
 "celebrity face, real person portrait, surveillance camera, cctv, military, weapon, soldier, flag closeup, scary, dark, ugly, deformed, extra fingers, blurry")
def eth(region):
...
CREAM=(16,20,30); NAVY=(233,206,140); CORAL=(206,58,48); INK=(238,234,226); PAPER=(28,34,48)
def F(p,s):
...
def tts(text,out):
    ...
    run([VENV,os.path.join(HERE,"_tts_dramatic.py"),tf,out])
def sdxl(scene,seed,out):
    g={"4":{"class_type":"CheckpointLoaderSimple","inputs":{"ckpt_name":CKPT}},
       ...
       "9":{"class_type":"SaveImage","inputs":{"filename_prefix":"kladSH","images":["8",0]}}}
...
def frame(short_title,head,caption,img,role,idx,n,out):
    ...
    im.paste(card,(cx0,cy0),mask); d.rounded_rectangle([cx0,cy0,cx1,cy1],radius=28,outline=(206,58,48),width=5)
    # 하단 자막 밴드
    d.rounded_rectangle([60,1360,1020,1810],radius=26,fill=PAPER,outline=(58,66,84),width=3)
    ...
    for i in range(n):
        c=CORAL if i==idx else (66,74,92)
...
if __name__=="__main__":
    ...
    for si,sh in enumerate(PLAN["shorts"]):
        ...
        out=f"{RENDER}/korea-lost-decade-{sh['key']}.mp4"
