#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""중국 AI교육 롱폼(4~6분) 빌드 — 플랜(us_ai_education_longform_plan.json) 기반.
일러스트(SDXL)+한글 오버레이(줌 위 정적)+약한 켄번즈+디졸브. 1280x720. 섹션 내레이션 싱크.
산출: assets/us-ai-education/render/us-ai-education-longform.mp4"""
...
A=f"{REPO}/assets/us-ai-education"; RENDER=f"{A}/render"; IMG=f"{A}/img"; OVL=f"{A}/ovl"; AUD=f"{A}/audio"
...
PLAN=json.load(open(f"{HERE}/us_ai_education_longform_plan.json",encoding="utf-8"))
STYLE=PLAN["style"]
...
def eth(key):
    ...
    return ("Korean East Asian people, Korean children and parents, black hair, " if key=="korea"
            else "American people, diverse ethnicities, multicultural children and teacher, ")
def F(p,s):
...
def sdxl(scene,seed,out):
    g={"4":{"class_type":"CheckpointLoaderSimple","inputs":{"ckpt_name":CKPT}},
       ...
       "9":{"class_type":"SaveImage","inputs":{"filename_prefix":"usLF","images":["8",0]}}}
...
if __name__=="__main__":
    ...
    for s in PLAN["sections"]:
        ...
        si+=1
    out=f"{RENDER}/us-ai-education-longform.mp4"
