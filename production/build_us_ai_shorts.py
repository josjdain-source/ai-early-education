#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""중국 AI교육 쇼츠 5편 빌드 — 플랜(us_ai_education_shorts_plan.json) 기반.
독립 후킹구조(hook/problem/fact/parent/cta). 1080x1920. 상단 큰 후킹문구·중앙 일러스트·하단 자막·빠른 디졸브.
산출: assets/us-ai-education/render/shorts/us-short-<s>.mp4"""
...
A=f"{REPO}/assets/us-ai-education"; RENDER=f"{A}/render/shorts"; IMG=f"{A}/simg"; FR=f"{A}/sframe"; AUD=f"{A}/saud"
for d in (RENDER,IMG,FR,AUD): os.makedirs(d,exist_ok=True)
KB=r"C:\Windows\Fonts\malgunbd.ttf"; KF=r"C:\Windows\Fonts\malgun.ttf"
PLAN=json.load(open(f"{HERE}/us_ai_education_shorts_plan.json",encoding="utf-8")); STYLE=PLAN["style"]
...
def eth(region):
    ...
    return ("Korean East Asian people, Korean children and parents, black hair, " if region=="kr"
            else "American people, diverse ethnicities, multicultural children, ")
...
def sdxl(scene,seed,out):
    g={"4":{"class_type":"CheckpointLoaderSimple","inputs":{"ckpt_name":CKPT}},
       ...
       "9":{"class_type":"SaveImage","inputs":{"filename_prefix":"usSH","images":["8",0]}}}
...
if __name__=="__main__":
    ...
    for si,sh in enumerate(PLAN["shorts"]):
        ...
        out=f"{RENDER}/us-short-{sh['key']}.mp4"
