...
A=f"{REPO}/assets/gwangju-semi"; RENDER=f"{A}/render"; IMG=f"{A}/img"; OVL=f"{A}/ovl"; AUD=f"{A}/audio"
...
PLAN=json.load(open(f"{HERE}/gwangju_semi_longform_plan.json",encoding="utf-8")); STYLE=PLAN["style"]
...
def sdxl(scene,seed,out):
    g={"4":{"class_type":"CheckpointLoaderSimple","inputs":{"ckpt_name":CKPT}},
       ...
       "9":{"class_type":"SaveImage","inputs":{"filename_prefix":"gjLF","images":["8",0]}}}
...
def register_to_studio(mp4, poster):
    ...
    desc=("광주 군공항 반도체 클러스터가 들어오면 지역경제·세수·주거가 어떻게 바뀔지, 평택 사례를 기준으로 본 긍정 시나리오입니다. ★공식 확정 예측이 아닙니다. 수치는 보도·통계 기준과 가정 시나리오입니다."+chr(10)+chr(10)+"#광주 #반도체 #평택 #지역경제 #군공항")
...
if __name__=="__main__":
    ...
    for s in PLAN["sections"]:
        ...
        si+=1
    out=f"{RENDER}/gwangju-semi-longform.mp4"
    RD.assemble(beats,auds,out,f"{A}/_dtmp")
    poster=f"{RENDER}/gwangju-semi-poster.jpg"
