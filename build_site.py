...
# ---------- 홈 ----------
def home():
    problem_cards="".join(f"""<div class="home-card problem-card">
<div class="home-icon">{ic}</div><h3>{title}</h3><p>{desc}</p></div>"""
        for ic,title,desc in [
        ("📋","AI 답을 그대로 베껴요","그럴듯한 문장을 자기 생각처럼 옮겨 적습니다."),
        ("🔎","틀린 답인지 확인하지 않아요","숫자, 사실, 이유가 맞는지 검토하지 않습니다."),
        ("💬","질문을 한 번 하고 끝내요","다시 묻고 비교하는 과정이 빠집니다."),
        ("🎮","재미로만 쓰고 생각 훈련은 안 돼요","놀이 경험은 있지만 판단 습관은 남지 않습니다."),
    ])
    lesson_steps="".join(f"""<div class="home-card lesson-step">
<div class="step-time">{time}</div><h3>{title}</h3><p>{desc}</p></div>"""
        for time,title,desc in [
        ("1분","아이가 AI에게 질문한다","숙제, 책, 뉴스, 그림 등 오늘 궁금한 것을 하나 묻습니다."),
        ("3분","AI 답에서 이상한 부분을 찾는다","말이 안 되는 부분, 빠진 근거, 확인할 사실을 표시합니다."),
        ("5분","다시 묻고 고친다","왜 그런지, 다른 답은 없는지, 근거는 무엇인지 다시 묻습니다."),
        ("1분","부모가 한 문장으로 정리한다","오늘 배운 판단 기준을 아이 말로 남깁니다."),
    ])
    world_cards="".join(f"""<a class="home-card evidence-card" href="{url}">
<div class="home-icon">{flag}</div><h3>{country}</h3><p>{text}</p></a>"""
        for flag,country,text,url in [
        ("🇬🇧","영국","안전, 책임, 비판적 사고를 AI 수업의 앞에 둡니다.","/world-cases/uk.html"),
        ("🇺🇸","미국","AI를 이해하고, 의심하며, 고쳐 쓰는 경험을 강조합니다.","/world-cases/usa.html"),
        ("🇨🇳","중국","학교 안으로 들어온 AI만큼 가정의 사용 습관이 중요해졌습니다.","/world-cases/china.html"),
        ("🌏","싱가포르·핀란드·호주 등","생활 속 실습과 안전한 판단 틀을 함께 다룹니다.","/world-cases.html"),
    ])
    body=f"""<main class="home-v2">
<style>
.home-v2 .home-hero{{background:linear-gradient(135deg,#FBF6EE 0%,#F6EEDF 58%,#FBE7DF 100%);border-bottom:1px solid var(--line);padding:72px 0 56px}}
.home-v2 .hero-inner{{display:grid;grid-template-columns:minmax(0,1.05fr) 420px;gap:34px;align-items:center}}
.home-v2 .eyebrow{{display:inline-flex;align-items:center;gap:8px;background:#fff;border:1px solid var(--line2);border-radius:999px;padding:7px 13px;color:var(--coral-d);font-weight:800;font-size:13px;margin-bottom:16px}}
.home-v2 h1{{font-size:48px;line-height:1.16;letter-spacing:-.02em;margin-bottom:18px}}
.home-v2 .hero-sub{{font-size:19px;color:var(--navy2);font-weight:700;max-width:660px;margin-bottom:10px}}
.home-v2 .hero-small{{color:var(--muted);font-weight:700;margin-bottom:26px}}
.home-v2 .hero-actions{{display:flex;gap:12px;flex-wrap:wrap}}
.home-v2 .hero-panel{{background:#fffdf8;border:1px solid #EAD9BE;border-radius:22px;padding:24px;box-shadow:var(--shadow)}}
.home-v2 .chat-row{{display:flex;gap:10px;margin-bottom:12px;align-items:flex-start}}
.home-v2 .chat-dot{{width:36px;height:36px;border-radius:12px;background:var(--coral);color:#fff;display:grid;place-items:center;font-weight:900;flex:0 0 auto}}
.home-v2 .chat-bubble{{background:#fff;border:1px solid var(--line2);border-radius:14px;padding:12px 14px;color:var(--ink);font-weight:700;font-size:14px;box-shadow:var(--shadow-sm)}}
.home-v2 .chat-row.alt .chat-dot{{background:var(--teal)}}
.home-v2 .chat-row.alt .chat-bubble{{background:#F0FAF7}}
.home-v2 .home-section{{padding:54px 0}}
.home-v2 .home-section.tight{{padding-top:24px}}
.home-v2 .section-head{{text-align:center;max-width:760px;margin:0 auto 24px}}
.home-v2 .section-head h2{{font-size:30px;margin-bottom:8px}}
.home-v2 .section-head p{{color:var(--muted);font-weight:650}}
.home-v2 .home-grid{{display:grid;gap:16px}}
.home-v2 .grid-3{{grid-template-columns:repeat(3,1fr)}}
.home-v2 .grid-4{{grid-template-columns:repeat(4,1fr)}}
.home-v2 .home-card{{background:#fff;border:1px solid var(--line2);border-radius:18px;padding:20px;box-shadow:var(--shadow-sm);text-decoration:none;color:inherit}}
.home-v2 a.home-card{{transition:.18s}}
.home-v2 a.home-card:hover{{transform:translateY(-3px);box-shadow:var(--shadow)}}
.home-v2 .home-icon{{width:48px;height:48px;border-radius:14px;background:var(--cream2);display:grid;place-items:center;font-size:24px;margin-bottom:12px}}
.home-v2 .home-card h3{{font-size:17px;margin-bottom:7px}}
.home-v2 .home-card p{{color:var(--muted);font-size:14px;margin:0}}
.home-v2 .problem-card{{border-top:4px solid var(--coral)}}
.home-v2 .conclusion{{text-align:center;background:#fff7ea;border:1px solid #EAD9BE;border-radius:18px;padding:18px 20px;margin-top:18px;color:#7a3e12;font-weight:800}}
.home-v2 .lesson-band{{background:#fffdf8;border-top:1px solid var(--line);border-bottom:1px solid var(--line)}}
.home-v2 .lesson-step{{position:relative;border-top:4px solid var(--teal)}}
.home-v2 .step-time{{display:inline-flex;background:#EAF5F2;color:#2f6f69;border-radius:999px;padding:4px 10px;font-weight:900;font-size:13px;margin-bottom:10px}}
.home-v2 .system-card:nth-child(1){{border-top:4px solid #2E8B96}}
.home-v2 .system-card:nth-child(2){{border-top:4px solid var(--coral)}}
.home-v2 .system-card:nth-child(3){{border-top:4px solid #7A5BB0}}
.home-v2 .evidence-card{{border-top:4px solid #E3B24C}}
.home-v2 .start-card{{display:flex;gap:14px;align-items:flex-start}}
.home-v2 .start-card .home-icon{{margin-bottom:0;flex:0 0 auto}}
.home-v2 .paid{{background:linear-gradient(135deg,#FFF7EA,#FBEBD3);border-top:1px solid var(--line)}}
.home-v2 .paid .home-card{{background:#fffdf8}}
.home-v2 .cta-center{{display:flex;justify-content:center;margin-top:22px}}
@media(max-width:920px){{.home-v2 .hero-inner{{grid-template-columns:1fr}}.home-v2 .hero-panel{{max-width:560px}}.home-v2 .grid-4{{grid-template-columns:repeat(2,1fr)}}.home-v2 .grid-3{{grid-template-columns:1fr}}}}
@media(max-width:640px){{.home-v2 .home-hero{{padding:48px 0 34px}}.home-v2 h1{{font-size:33px}}.home-v2 .hero-sub{{font-size:16px}}.home-v2 .hero-actions .btn{{width:100%;justify-content:center}}.home-v2 .grid-4{{grid-template-columns:1fr}}.home-v2 .section-head h2{{font-size:24px}}}}
</style>
<section class="home-hero"><div class="wrap hero-inner">
<div>
<div class="eyebrow">부모와 아이가 집에서 시작하는 AI 판단 훈련</div>
<h1>AI가 대신 생각해주는 아이가 아니라,<br><span class="coral">AI 답을 의심하고 고치는 아이로.</span></h1>
<p class="hero-sub">아이에게 필요한 것은 프롬프트 기술이 아니라,<br>AI가 틀렸을 때 다시 묻고, 비교하고, 판단하는 힘입니다.</p>
<p class="hero-small">유치원생부터 고등학생까지, 집에서 바로 하는 AI 대화 훈련</p>
<div class="hero-actions">
<a class="btn btn-primary btn-lg" href="/today">오늘 10분 수업 시작하기</a>
<a class="btn btn-ghost btn-lg" href="/levels">우리 아이 나이별 로드맵 보기</a>
<a class="btn btn-navy btn-lg" href="https://github.com/josjdain-source/ai-early-education" target="_blank" rel="noopener">GitHub에서 프로젝트 보기</a>
</div>
</div>
<div class="hero-panel" aria-label="AI 답을 의심하고 고치는 대화 예시">
<div class="chat-row"><div class="chat-dot">AI</div><div class="chat-bubble">"북극곰은 펭귄을 자주 사냥합니다."</div></div>
<div class="chat-row alt"><div class="chat-dot">아이</div><div class="chat-bubble">잠깐, 펭귄은 남극에 살잖아. 이 답은 이상해.</div></div>
<div class="chat-row"><div class="chat-dot">AI</div><div class="chat-bubble">맞아요. 지역을 혼동했습니다. 다시 설명할게요.</div></div>
<div class="chat-row alt" style="margin-bottom:0"><div class="chat-dot">부모</div><div class="chat-bubble">오늘의 한 문장: AI 답도 근거와 위치를 확인한다.</div></div>
</div>
</div></section>

<section class="home-section"><div class="wrap">
<div class="section-head"><h2>혹시 아이가 이렇게 AI를 쓰고 있나요?</h2></div>
<div class="home-grid grid-4">{problem_cards}</div>
<div class="conclusion">이 사이트는 아이에게 AI 사용법보다 먼저,<br>AI 답을 다루는 법을 가르칩니다.</div>
</div></section>

<section class="home-section lesson-band"><div class="wrap">
<div class="section-head"><h2>오늘 집에서 바로 해보는 10분 AI 수업</h2><p>길게 설명하지 않아도 됩니다. 오늘 질문 하나로 의심하고 고치는 연습을 시작합니다.</p></div>
<div class="home-grid grid-4">{lesson_steps}</div>
<div class="cta-center"><a class="btn btn-primary btn-lg" href="/today">오늘 수업 보기</a></div>
</div></section>

<section class="home-section"><div class="wrap">
<div class="section-head"><h2>우리 집 3층 교실 시스템</h2><p>근거를 보고, 오늘 해보고, 아이 나이에 맞게 반복합니다.</p></div>
<div class="home-grid grid-3">
<a class="home-card system-card" href="/world-cases.html"><div class="home-icon">🌍</div><h3>세계 AI교육에서 배운 원칙</h3><p>AI를 많이 쓰는 법보다 안전하게 의심하고 판단하는 원칙을 먼저 봅니다.</p></a>
<a class="home-card system-card" href="/today"><div class="home-icon">🏠</div><h3>오늘 집에서 하는 10분 수업</h3><p>질문, 오류 찾기, 다시 묻기, 부모 정리까지 한 번에 실행합니다.</p></a>
<a class="home-card system-card" href="/levels"><div class="home-icon">🎯</div><h3>우리 아이 나이에 맞는 로드맵</h3><p>유치원부터 고등까지 말하기, 고치기, 의심, 검증, 해결로 확장합니다.</p></a>
</div></div></section>

<section class="home-section tight"><div class="wrap">
<div class="section-head"><h2>세계는 이미 AI를 ‘쓰는 법’보다 ‘판단하는 법’을 가르칩니다</h2><p>세계 AI교육은 첫 화면의 주인공이 아니라, 부모가 오늘 시작해도 되는 신뢰 근거입니다.</p></div>
<div class="home-grid grid-4">{world_cards}</div>
<div class="cta-center"><a class="btn btn-ghost" href="/world-cases.html">세계 AI교육 전체 보기</a></div>
</div></section>

<section class="home-section"><div class="wrap">
<div class="section-head"><h2>처음이라면 무료로 시작하세요</h2><p>자료를 받고, 나이별 목표를 확인한 뒤, 부모 질문법으로 오늘 대화를 이어갑니다.</p></div>
<div class="home-grid grid-3">
<a class="home-card start-card" href="/free-kit"><div class="home-icon">🎁</div><div><h3>무료 시작 키트</h3><p>인쇄해서 바로 쓰는 질문 카드와 활동지를 모았습니다.</p></div></a>
<a class="home-card start-card" href="/levels"><div class="home-icon">🧭</div><div><h3>나이별 로드맵</h3><p>우리 아이에게 지금 필요한 AI 대화 훈련을 확인합니다.</p></div></a>
<a class="home-card start-card" href="/parents"><div class="home-icon">👨‍👩‍👧</div><div><h3>부모 질문법</h3><p>정답을 알려주기보다 다시 묻게 하는 부모 멘트를 익힙니다.</p></div></a>
</div></div></section>

<section class="home-section paid"><div class="wrap">
<div class="section-head"><h2>반복해서 운영하고 싶다면</h2><p>가정, 교실, 채널 운영자가 매주 콘텐츠와 수업 흐름을 만들 수 있게 돕습니다.</p></div>
<div class="home-grid grid-3">
<a class="home-card" href="/paid-programs#shorts-builder"><div class="home-icon">🎬</div><h3>쇼츠 자동 빌더</h3><p>AI교육 메시지를 짧은 영상 흐름으로 빠르게 구성합니다.</p></a>
<a class="home-card" href="/paid-programs#video-maker"><div class="home-icon">📽️</div><h3>영상 자동 생성기</h3><p>대본, 장면, 이미지, 영상 제작을 한 번의 운영 흐름으로 묶습니다.</p></a>
<a class="home-card" href="/paid-programs#workbook-guide"><div class="home-icon">📝</div><h3>12주 부모 해설판</h3><p>반복 수업을 부모 설명, 질문, 체크포인트와 함께 운영합니다.</p></a>
</div>
<div class="cta-center"><a class="btn btn-primary btn-lg" href="/paid-programs">유료 프로그램 보기</a></div>
</div></section>
</main>"""
    return page("home","",
      "AI 답을 의심하고 고치는 아이로 | AI 조기교육",
      "부모가 집에서 아이와 AI 답을 의심하고 고치는 훈련을 시작하는 AI 조기교육 사이트. 오늘 10분 수업, 나이별 로드맵, 무료 시작 키트.",body)
...
<p style="font-size:12.5px;color:var(--muted);margin-top:8px">👨‍👩‍👧 아이와 함께 쓰는 질문카드·활동지는 <a href="/parent-resources.html" style="color:var(--coral);font-weight:700">부모 자료실</a>에 있습니다. 이 방은 '만드는 사람'용 도구방입니다. &nbsp;💾 우리가 직접 개발한 도구는 <a href="/paid-programs.html" style="color:var(--coral);font-weight:700">유료 프로그램(출시 전 상담)</a>에.</p>
{secs}
